...
def train(config, betas, num_timesteps, epoch, num_epoch, epoch_iters, base_lr, num_iters,
    ...
    for i_iter, batch in enumerate(trainloader):
        ...
        if config.LOSS.USE_LAYER_LOSS:
            output, jac_loss, _, output_zm = model(xt, t.float(), train_step=(lr_scheduler._step_count-1), 
                                compute_jac_loss=compute_jac_loss,
                                f_thres=f_thres, b_thres=b_thres, writer=writer)
        else:
            output, jac_loss, _, _ = model(xt, t.float(), train_step=global_steps, 
                                       compute_jac_loss=compute_jac_loss,
                                       f_thres=f_thres, b_thres=b_thres, writer=writer)
        
        losses = (e - output).square().sum(dim=(1, 2, 3))
        if config.LOSS.USE_LAYER_LOSS and len(output_zm) > 0:
            assert type(config.LOSS.GAMMA) == list and len(config.LOSS.GAMMA) == len(output_zm)
            
            gamma = config.LOSS.GAMMA
            for i, layer_output in enumerate(output_zm):
                losses += gamma[i] * (layer_output - output).square().sum(dim=(1, 2, 3))
        
        if config.LOSS.USE_IMAGE_LOSS:
            pred_x = (xt - output * (1.0 - a).sqrt()) / a.sqrt()
            losses += config.LOSS.LAMBDA * (pred_x - x).square().sum(dim=(1, 2, 3))
