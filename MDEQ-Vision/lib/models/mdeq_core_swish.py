...
class Stage0Block(nn.Module):
    def __init__(self, in_channels, out_channels=None, conv_shortcut=False,
        ...
        self.stage0_0 = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False),
                                        #nn.GroupNorm(NUM_GROUPS, out_channels, affine=BLOCK_GN_AFFINE)
...
class MDEQDiffNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.downsample = nn.Sequential(
            ...
            nn.GroupNorm(NUM_GROUPS, init_chansize, affine=BLOCK_GN_AFFINE),
        ...
        if self.downsample_times > 2:
            for i in range(3, self.downsample_times+1):
                ...
                self.downsample.add_module(f"DS{i}-GN", nn.GroupNorm(NUM_GROUPS, init_chansize, affine=BLOCK_GN_AFFINE)(init_chansize, momentum=BN_MOMENTUM, affine=True))
    ...
    def _forward(self, x, temb, train_step=-1, compute_jac_loss=True, spectral_radius_mode=False, writer=None, **kwargs):
        """
