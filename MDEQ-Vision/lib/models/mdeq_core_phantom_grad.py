...
class MDEQDiffNet(nn.Module):
    ...
    def parse_cfg(self, cfg):
        ...
        # Parameters for inexact gradients
        self.tau = cfg['DEQ']['TAU']
        self.iter_steps = cfg['DEQ']['ITER_STEPS']
        ...
        if not deq_mode:
            ...
            if self.training:
                for _ in range(self.iter_steps):
                    z1 = (1 - self.tau) * z1 + self.tau*func(z1.requires_grad_())

        y_list = self.iodrop(vec2list(z1, cutoffs))
