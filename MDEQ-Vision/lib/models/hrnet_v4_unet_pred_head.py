...
class HighResolutionNetV3(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.attn = AttnBlock(num_channels[1])

        self.incre_modules, self.up_modules = self._make_head(pre_stage_channels)
    ...
    def predict_noise(self, y_list, temb):
        ...
        for i in range(len(y_list) - 1, -1, -1):
            if i == len(y_list) - 1:
                y = self.incre_modules[i]((y_list[i], temb))[0]
            else: 
                y = self.incre_modules[i]((torch.cat((y_list[i], y), dim=1), temb))[0]
