...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, temb_channels=512):
        ...
        self.gn1 = nn.GroupNorm(num_groups=32, num_channels=planes, eps=1e-6, affine=True)
        self.conv2 = conv3x3(planes, planes)
        self.gn2 = nn.GroupNorm(num_groups=32, num_channels=planes, eps=1e-6, affine=True)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, temb_channels=512):
        ...
        self.bn1 = nn.GroupNorm(num_groups=32, num_channels=planes, eps=1e-6, affine=True)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.GroupNorm(num_groups=32, num_channels=planes, eps=1e-6, affine=True)
        self.conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1,
                               bias=False)
        self.bn3 = nn.GroupNorm(num_groups=32, num_channels=planes * self.expansion, eps=1e-6, affine=True)
        self.downsample = downsample
        self.stride = stride
...
class HighResolutionNetV3(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.GroupNorm(num_groups=32, num_channels=64, eps=1e-6, affine=True)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn2 = nn.GroupNorm(num_groups=32, num_channels=64, eps=1e-6, affine=True)
        ...
        self.last_layer = nn.Sequential(
            nn.Conv2d(
                ...
                stride=1,
                padding=0),
            nn.GroupNorm(num_groups=32, num_channels=last_inp_channels, eps=1e-6, affine=True),
            nn.SiLU(),
            nn.Conv2d(
                in_channels=last_inp_channels,
                out_channels=3,
            ...
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        nn.Conv2d(num_channels_pre_layer[i],
                                  ...
                                  1,
                                  bias=False),
                        nn.GroupNorm(num_groups=32, num_channels=num_channels_cur_layer[i], eps=1e-6, affine=True),
                        nn.SiLU()))
                else:
                ...
                for j in range(i+1-num_branches_pre):
                    ...
                    conv3x3s.append(nn.Sequential(
                        ...
                        nn.GroupNorm(num_groups=32, num_channels=outchannels, eps=1e-6, affine=True),
                        nn.SiLU()))
