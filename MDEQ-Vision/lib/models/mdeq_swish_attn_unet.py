...
from mdeq_core_swish_attn_unet import MDEQDiffNet, nonlinearity, SwishActivation, AttnBlock
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, temb_channels=512):
        ...
        self.gn1 = nn.GroupNorm(NUM_GROUPS, planes, affine=POST_GN_AFFINE)
        
        self.conv2 = conv3x3(planes, planes, stride=stride)
        self.gn2 = nn.GroupNorm(NUM_GROUPS, planes, affine=POST_GN_AFFINE)
        
        self.conv3 = nn.Conv2d(planes, planes*self.expansion, kernel_size=1, bias=False)
        self.gn3 = nn.GroupNorm(NUM_GROUPS, planes*self.expansion, affine=POST_GN_AFFINE)
...
class MDEQDiffusionNet(MDEQDiffNet):
    def __init__(self, cfg, **kwargs):
        ...
        last_inp_channels = np.int(self.num_channels[1])
    ...
    def _make_head(self, pre_stage_channels):
        ...
        for i, channels in enumerate(pre_stage_channels):
            if i == len(pre_stage_channels)-1:
                incre_module = self._make_layer(head_block, channels, channels, blocks=1, stride=1)
            else:
                in_channels = pre_stage_channels[i] + pre_stage_channels[i+1]
                incre_module = self._make_layer(head_block, in_channels, channels, blocks=1, stride=1)
        ...
        for i, channels  in enumerate(pre_stage_channels):
            if i == len(pre_stage_channels)-1:
                upsample_module = Upsample(channels)
            else:
                upsample_module = Upsample(channels)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, padding=0):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride, bias=False, padding=padding),
                                       nn.GroupNorm(NUM_GROUPS, planes, affine=POST_GN_AFFINE),
    ...
    def predict_noise(self, y_list):
        ...
        Combine all resolutions and output noise
        """
        y = self.noise_pred_layer(y_list[1])
        return y
    ...
    def forward(self, x, t, train_step=0, **kwargs):
        ...
        if output_zm is not None and len(output_zm) > 0:
            noise_zm = []
            for cur_output_zm in output_zm:
                pred_zm = self.predict_noise(cur_output_zm)
